...
public class LeftJoystick : MonoBehaviour, IDragHandler, IPointerUpHandler, IPointerDownHandler, IEventSystemHandler
{
	...
	private void Start()
	{
		...
		if (GetComponent<Image>() != null && base.transform.GetChild(0).GetComponent<Image>() != null)
		{
			...
			bgImage.rectTransform.anchorMin = new Vector2(0f, 0f);
			bgImage.rectTransform.anchorMax = new Vector2(0f, 0f);
	...
	public virtual void OnDrag(PointerEventData ped)
	{
		...
		if (RectTransformUtility.ScreenPointToLocalPointInRectangle(bgImage.rectTransform, ped.position, ped.pressEventCamera, out localPoint))
		{
			...
			inputVector = ((!((double)inputVector.magnitude > 0.25)) ? inputVector : (inputVector.normalized / 4f));
			...
			if (!joystickStaysInFixedPosition && unNormalizedInput.magnitude > inputVector.magnitude)
			{
				...
				position.x = Mathf.Clamp(x5, sizeDelta5.x, Screen.width / 2);
				float y4 = position.y;
				float num = Screen.height;
				Vector2 sizeDelta6 = bgImage.rectTransform.sizeDelta;
				position.y = Mathf.Clamp(y4, 0f, num - sizeDelta6.y);
